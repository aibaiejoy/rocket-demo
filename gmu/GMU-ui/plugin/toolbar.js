...
(function($) {
     ...
     * - *** buttonText {String}*** (可选)返回按钮文字
     * - *** buttonClick {Function}*** (可选)返回按钮的点击事件
     * - *** showType {Number}*** (可选)显示类型；1,下拉菜单方式，2，二级工具条
     * - *** tools {Array}*** (可选)工具的数组,每个是一个工具
     * - *** tools[x].class {String}*** (可选)工具的class（css）名称
     * - *** tools[x].click {Function}*** (可选)工具的点击事件
     * - *** onverticalswipedown {Event}*** (可选)组件上向下滑动时触发
     * - *** onverticalswipeup {Event}*** (可选)组件上向上滑动时触发
     * - *** isPreventDefault {Boolean}*** (可选)是否取消move事件的默认行为
     * @name     $.ui.toolbar
     * @grammar  $.ui.toolbar([el [,options]])  ⇒ instance
     * @example
     * var toolbarIns = $.ui.toolbar({
        instanceId:'blue',//针对不同的实例，可以写不同的css
        showType:2,
        titleText:'焦点新闻',
        buttonText:'返回', //实时,String,按钮文字
        oncreate: function(){
            this.widget().find('.ui-toolbar-backbtn').behavior('ui-toolbar-backbtn-hover');
        },
     * 
     * @mode render模式
    ...
    $.ui.create('toolbar', {
         ...
         * - 如果设置参数，设置返回按钮的文字为参数
         * @name buttonText
         * @grammar  buttonText()  ⇒ string
         * @grammar  buttonText(value)  ⇒ self
         * @example
         * toolbarIns.buttonText();
         * toolbarIns.buttonText("value");
         */
        buttonText: function() {
            var me = this,
                    $elem = me.widget();
         ...
         * - ***create*** : 组件创建时触发
         * - ***init*** : 组件初始化时触发
         * - ***verticalswipedown*** : 向下竖滑触发的事件
         * - ***verticalswipeup*** : 向上竖滑触发的事件
         * - ***beforeshow*** : 显示之前触发的事件
         * - ***aftershow*** : 显示之后触发的事件
         * - ***beforehide*** : 隐藏之前触发的事件
         * - ***afterhide*** : 隐藏之后触发的事件
         * @example
         * $.ui.refresh(opts).on('verticalswipedown', function () {
         *      console.log('verticalswipedown');
         * });
         */
